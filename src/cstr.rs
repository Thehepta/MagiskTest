use crate::cstr::fmt::Debug;
use crate::dir::Directory;
use crate::result::{LibcReturn, OsError, OsResult, OsResultStatic};
use core::fmt;
use libc::{mode_t, open, EEXIST};
use nix::errno::{errno, Errno};
use std::borrow::Borrow;
use std::cmp::min;
use std::cmp::Ordering;
use std::ffi::OsStr;
use std::ffi::{c_char, c_uint, CStr, FromBytesWithNulError};
...
macro_rules! cstr {
    ($str:expr) => {{
        const NULL_STR: &str = const_format::concatcp!($str, "\0");
        #[allow(unused_unsafe)]
        unsafe {
            Utf8CStr::from_bytes_unchecked(NULL_STR.as_bytes())
        }
    }};
}
...
impl Utf8CStr {
    ...
    pub fn as_str(&self) -> &str {
        ...
        unsafe { std::str::from_utf8_unchecked(self.0.get_unchecked(..self.0.len() - 1)) }
    }
}
